...
private:
    ...
    bool load_vectors_mmap(std::ifstream& ifs, size_t num_vectors) {
        ...
        size_t relative_offset = 0;  // Offset relative to vectors_file_offset_
        
        for (size_t i = 0; i < num_vectors; i++) {
            labeltype label;
            ifs.read(reinterpret_cast<char*>(&label), sizeof(labeltype));
            
            // Store the offset where THIS label's data starts (relative to vectors_file_offset_)
            ...
            // Move offset forward by size of label + vector
            relative_offset += sizeof(labeltype) + dim_ * sizeof(float);
        }
        
        // Now map the actual file
        #ifdef _WIN32
    ...
    bool setup_mmap_unix() {
        ...
        // Map the ENTIRE file into memory and use the offset

        // The OS will handle paging in/out as needed
    ...
    bool setup_mmap_windows() {
        ...
        // Can close handles after mapping
...
private:
    const float* get_vector_from_mmap(labeltype label) const {
        ...
        // vectors_file_offset_ = where vector section starts in file
        // it->second = offset of this label's data relative to vector section start
        // + sizeof(labeltype) = skip past the label to get to the vector data
        size_t absolute_offset = vectors_file_offset_ + it->second + sizeof(labeltype);
        
        // Bounds check to prevent reading past end of mapped region
        if (absolute_offset + dim_ * sizeof(float) > mmap_size_) {
            return nullptr;
        }
        
        // Return pointer directly into mapped memory at the calculated offset
        return reinterpret_cast<const float*>(
            static_cast<char*>(mmap_ptr_) + absolute_offset
        );
    }
