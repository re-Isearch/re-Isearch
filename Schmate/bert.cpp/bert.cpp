...
to support the latest ggml library (0.11.1) of May 2026
...
What this interface does NOT support are ome of the newer models such as
...
// ---------------------------------------------------------------------------
// GGUF metadata helpers (translation-unit private)
// ---------------------------------------------------------------------------
...
static const char * get_string(const struct gguf_context * const ctx, const std::string & key) {
    // 1. Locate the key's internal ID index
    int key_id = gguf_find_key(ctx, key.c_str());
    
    // 2. Return an empty view if the key isn't in this GGUF file or not a string
    if (key_id < 0 || gguf_get_kv_type(ctx, key_id) != GGUF_TYPE_STRING)
        return nullptr;
    
    // 3. Extract the underlying const char*
    return gguf_get_val_str(ctx, key_id);
}

BERT_API const char * bert_get_model_name(const bert_ctx * ctx) {
   return get_string(ctx->ctx_meta, "general.name");
}

BERT_API const char * bert_get_architecture(const bert_ctx * ctx) {
   return get_string(ctx->ctx_meta, "general.architecture");
}
...
// Try every name; on failure dump the first 50 tensor names from the file.
...
bert_tokens bert_tokenize(bert_ctx * ctx, bert_string text, uint64_t n_max_tokens) {
    ...
    for (const auto & word : words) {
        ...
        auto * tmap = &vocab.token_to_id;
    loop:
        if (tokens.size() >= n_max_tokens - 1) break;
        while (i < n) {
            for (int j = n; j > i; --j) {
                auto it = tmap->find(word.substr(i, j-i));
                if (it != tmap->end()) {
                    tokens.push_back(it->second);
                    match = true; i = j;
                    tmap = &vocab.subword_token_to_id;
                    goto loop;
                }
            }
            tmap = &vocab.subword_token_to_id; ++i;
        }
        if (!match) tokens.push_back(UNK);
    }
...
static int get_best_gpu_index(const std::string& backend_type) {
    ...
    if (backend_type == "CUDA") {
	    ...
	    ggml_log_internal(GGML_LOG_LEVEL_INFO, "CUDA Device %d: %s (%.2f GB VRAM)\n", i, desc, total / 1e9);
            if (total > max_vram) {
                max_vram = total;
                best_idx = i;
            }
	...
	ggml_log_internal(GGML_LOG_LEVEL_INFO, "Vulkan Device %d: %s | Total VRAM: %.2f GB\n", 
                i, desc, total / 1024.0 / 1024.0 / 1024.0);

        // Logic: Pick the one with the most VRAM
        if (total > max_vram) {
            max_vram = total;
            best_idx = i;
        }
...
struct bert_ctx * bert_load_from_file(const char * fname, bool use_cpu) {
    ...
    if (!ctx_gguf) {
	ggml_log_internal(GGML_LOG_LEVEL_ERROR, "%s: failed to open '%s'\n", __func__, fname);
        return nullptr;
    }
    ...
    // vocabulary
    {
        ...
        if (v_id != -1) {
            ...
            for (int i = 0; i < n; ++i) {
                vocab.tokens[i] = gguf_get_arr_str(ctx_gguf, v_id, i);
                const std::string & tok = vocab.tokens[i];
                if (tok.size() > 2 && tok.substr(0,2) == "##") {
                    const std::string sub = tok.substr(2);
                    vocab.subword_token_to_id[sub] = i;
                    vocab._id_to_subword_token[i]  = sub;
                } else {
...
#ifdef GGML_USE_CUDA
    if (!use_cpu) {
	int best_cuda = get_best_gpu_index("CUDA");
        bctx->backend = ggml_backend_cuda_init(best_cuda);
        if (bctx->backend)
	  ggml_log_internal(GGML_LOG_LEVEL_INFO, "%s: using CUDA device %d\n", __func__, best_cuda);
    }
#endif
#ifdef GGML_USE_METAL
    if (!use_cpu && !bctx->backend) {
        ...
        if (bctx->backend)
	  ggml_log_internal(GGML_LOG_LEVEL_INFO,"%s: using Metal backend\n", __func__);
    }
#endif
#ifdef GGML_USE_VULKAN 
    if (!use_cpu && !bctx->backend) {
	int best_vulkan =  get_best_gpu_index("VULKAN");
        bctx->backend = ggml_backend_vk_init(best_vulkan);
        if (bctx->backend)
	  ggml_log_internal(GGML_LOG_LEVEL_INFO, "%s: using Vulkan backend, device %d\n", __func__, best_vulkan);
    }
#endif
    if (!bctx->backend) {
        bctx->backend = ggml_backend_cpu_init();
	ggml_log_internal(GGML_LOG_LEVEL_INFO, "%s: using CPU backend\n", __func__);
    }
	...
	ggml_log_internal(GGML_LOG_LEVEL_ERROR, "%s: failed to allocate weights buffer\n", __func__);
        bert_free(bctx);
        return nullptr;
    }
    ...
    // read weight data from file
    {
        ...
        if (!fin) {
            ggml_log_internal(GGML_LOG_LEVEL_ERROR, "%s: cannot open '%s'\n", __func__, fname);
            bert_free(bctx);
            return nullptr;
        }
        for (int i = 0; i < n_tensors; ++i) {
...
void bert_allocate_buffers(bert_ctx * ctx, int32_t n_max_tokens, int32_t batch_size) {
    ...
    if (!ggml_gallocr_reserve(ctx->compute_alloc, gf)) {
        ggml_log_internal(GGML_LOG_LEVEL_ERROR, "%s: ggml_gallocr_reserve failed\n", __func__);
        return;
    }

    if (verbosity >= 1) {
        ggml_log_internal(GGML_LOG_LEVEL_INFO, "%s: compute buffer: %.2f MB\n", __func__,
...
void bert_deallocate_buffers(bert_ctx * ctx) {
    // 1. Free the graph allocator first
    if (ctx->compute_alloc) {
        ggml_gallocr_free(ctx->compute_alloc);
        ctx->compute_alloc = nullptr;
    }
    // 2. Free the compute buffer
...
void bert_free(bert_ctx * ctx) {
    ...
    // 1. Clear the graph allocator (transient GPU memory)
    if (ctx->compute_alloc) {
        ggml_gallocr_free(ctx->compute_alloc);
        ctx->compute_alloc = nullptr;
    }

    // 2. Clear the transient compute buffer
    if (ctx->compute_buffer) {
        ggml_backend_buffer_free(ctx->compute_buffer);
        ctx->compute_buffer = nullptr;
    }

    // 3. THE CULPRIT: Free the weights buffer explicitly 
    // This removes the weights from the Metal Residency Set.
    if (ctx->weights_buffer) {
        ggml_backend_buffer_free(ctx->weights_buffer);
        ctx->weights_buffer = nullptr;
    }

    // 4. Free the metadata context
    if (ctx->ctx_data) {
        ggml_free(ctx->ctx_data);
        ctx->ctx_data = nullptr;
    }

    // 5. NOW it is safe to kill the backend
    if (ctx->backend) {
...
void bert_free(bert_ctx * ctx) {
    bert_deallocate_buffers(ctx);
    if (ctx->weights_buffer) { ggml_backend_buffer_free(ctx->weights_buffer); ctx->weights_buffer = nullptr; }
    if (ctx->ctx_data)        { ggml_free(ctx->ctx_data);                      ctx->ctx_data        = nullptr; }
...
struct ggml_cgraph * bert_build_graph(bert_ctx * ctx, bert_batch batch) {
    ...
    int cur_max_len = 0;
    for (const auto & seq : batch) cur_max_len = std::max(cur_max_len, (int)seq.size());

    if (cur_max_len > n_max_tokens) {
        ggml_log_internal(GGML_LOG_LEVEL_ERROR, "Too many tokens: max %d, got %d\n", n_max_tokens, cur_max_len);
        return nullptr;
    }
    ...
    inpL = ggml_reshape_3d(ctx0, inpL, n_embd, cur_max_len, n_batch);
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // output projection + residual + post-attention norm
    ...
    // mean pool + L2-norm
    inpL = ggml_mul_mat(ctx0, ggml_cont(ctx0, ggml_transpose(ctx0, inpL)), sum);
...
void bert_forward_batch(bert_ctx * ctx, bert_batch batch,
    ...
    if (!ggml_gallocr_alloc_graph(ctx->compute_alloc, gf)) {
        fprintf(stderr, "%s: gallocr_alloc_graph failed\n", __func__);
        return;
    }

    // fill input tensors (they are the first 6 leaves of the graph)
    const int n_batch   = (int)batch.size();
    int cur_max_len = 0;
    for (const auto & s : batch) cur_max_len = std::max(cur_max_len, (int)s.size());

    // struct ggml_tensor * token_layer = ggml_graph_node(gf, 0);
...

    {
        ...
        std::vector<float>   pm(cur_max_len * n_batch);
        std::vector<int32_t> pos(cur_max_len * n_batch);
        std::vector<float>   sv(cur_max_len * n_batch);
        const float m1 = -1.0f;
        ...
        for (int ba = 0; ba < n_batch; ++ba) {
            ...
            for (int i = 0; i < cur_max_len; ++i) {
                ...
                if (i < cl) {
                    tl[idx] = batch[ba][i]; pm[idx] = 1.0f; sv[idx] = 1.0f/(float)cl;
                } else {
                    tl[idx] = 101;          pm[idx] = 0.0f; sv[idx] = 0.0f;
                }
                pos[idx] = i;
            }
        }
    ...
    if (ggml_backend_is_metal(ctx->backend)) {
       // NO LONGER NEEDED
       //  ggml_backend_metal_set_n_threads(ctx->backend, n_threads);
    }
#endif
...
void bert_encode_batch(bert_ctx * ctx, bert_strings texts,
    ...
    for (const auto & t : texts) batch.push_back(bert_tokenize(ctx, t, N));
...
const char * bert_vocab_id_to_token(bert_ctx * ctx, int32_t id) {
    ...
    it = v._id_to_subword_token.find(id);
    if (it != v._id_to_subword_token.end()) return it->second.c_str();
...
// If routing through BGE v1.5, alter the text dynamically before vectorization:
if (current_model == "bge-large-en-v1.5") {
    user_query = "Represent this sentence for searching relevant passages: " + user_query;
}

// Now encode and query your existing HNSW network
auto query_vector = my_bert_engine.encode(user_query);
auto results = hnsw_net.search(query_vector, top_k);
