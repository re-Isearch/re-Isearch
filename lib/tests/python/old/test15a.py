...
sentence = sys.argv[1:] and sys.argv[1] or 'out spot PEER'
...
query=QUERY();
squery=SQUERY();
squery.SetQueryTerm(sentence);
query.SetSQUERY(squery);
query.SetSortBy(ByScore);

irset = pdb.SearchSmart(query);

rpn =  query.GetSQUERY().GetRpnTerm();
if irset == None:
  print "Search for", rpn, "failed:", pdb.GetErrorCode(), pdb.ErrorMessage();
  sys.exit(-1);

total = irset.GetTotalEntries();
print "Smart searching for: ", sentence, "[",rpn,"]";
