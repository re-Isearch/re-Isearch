...
const char *XPANDOC::GetDefaultFilter() const  { return "pandoc"; }
const char *XPANDOC::GetDefaultOptions() const { return "-s -t docbook5";}
...
const char *XPANDOC::Description(PSTRLIST List) const
{
  static STRING help;

  const STRING ThisDoctype("XPANDOC");
...
void XPANDOC::SourceMIMEContent(STRING *stringPtr) const
{
  *stringPtr =  MIME_Type;
}

STRING  XPANDOC::SetFilter(const STRING& arg)
{
  STRING filter (arg);
...
XPANDOC::XPANDOC(PIDBOBJ DbParent, const STRING& Name) : XML(DbParent, Name)
{
...
//
GDT_BOOLEAN XPANDOC::GenRecord(const RECORD& FileRecord)
{
...
void XPANDOC::ParseRecords(const RECORD& FileRecord)
{
...
void XPANDOC::ParseFields(RECORD *RecordPtr)
{
...
void XPANDOC:: DocPresent (const RESULT& ResultRecord,
...
void XPANDOC::Present (const RESULT& ResultRecord,
         const STRING& ElementSet, const STRING& RecordSyntax,
         PSTRING StringBuffer) const
{
  StringBuffer->Clear();
...
GDT_BOOLEAN XPANDOC::GetResourcePath(const RESULT& ResultRecord, STRING *StringBuffer) const
{
