...
//cerr << "IDBOBJ::GetFieldType(" << FieldName << ")=" << s <<  " int=" << s.GetInt() << endl;
...
STRING IDBOBJ::ResolvePathname(const STRING& Path) const
{
  if (IsAbsoluteFilePath( Path ) || WorkingDirectory.IsEmpty())  
    return Path;
cerr << "WorkingDirectory=" << WorkingDirectory << endl;
cerr << "Path =" << Path << endl;
cerr << "---> " << __Realpath (AddTrailingSlash(WorkingDirectory) + Path) << endl;
