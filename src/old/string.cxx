...
/* ########################################################################
  ...
  This module is licensed to CNIDR/MCNC for use in Isite/Isearch. If you want
  to use this in other programs or projects please contact BSn.
...
void STRING::Clear()
{
  ...
  if ( ptr == NULL)
    {
      logf(LOG_PANIC, "String data corruption in Clear(). Call BSn or your vendor!");
    }
...
STRING STRING::SubString(const STRINGINDEX From, const STRINGINDEX To) const
{
  STRING dest;
  const size_t len = GetStringData()->nDataLength;
  if (len && (From <= To || To == 0))
    {
      const size_t start = (From > 0 ? From - 1 : 0);
