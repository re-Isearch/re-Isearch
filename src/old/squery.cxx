#pragma ident  "@(#)squery.cxx  1.64 02/05/01 00:33:31 BSN"
/* ########################################################################
      ...
      if (!Ored)
	      ...
	      if (op_count == 1)
		{
		 if (Op ==  OperatorWithinFile || Op == OperatorWithinFileExtension ||
			Op == OperatorWithinKey || OperatorWithinDoctype)
		    special = GDT_TRUE;
		}
            }
          else
  ...
  while (Stack >> OpPtr)
    {
      if (OpPtr->GetOpType () == TypeOperator)
		...
		op_t == OperatorBoostScore )
          ...
          else if (op_t == OperatorWithinFile || op_t == OperatorWithinKey ||
		   op_t == OperatorSortBy ||
