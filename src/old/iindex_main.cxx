...
Modifications:	Edwarc C. Zimmermann, edz@bsn.com
@@@-*/
...
#endif
      std::cout << endl
	<<"Copyright \251 1995-2008 Basis Systeme netzwerk/Munich. All Rights Reserved." << endl << endl;
      Usage();
    }
...
#endif
	               "  Max Words:   " << ((MAX_GPTYPE)/(2*sizeof(GPTYPE)*1024LL*1024LL)) << " million (optimized), >" <<
				((MAX_GPTYPE)/(2UL*1024UL*1024UL*1024UL)) << " billion (non-optimized)" << endl <<
		       "  Max Unique:  " << ((MAX_GPTYPE)/(1024L*1024L*2*(StringCompLength + sizeof(GPTYPE)+1))) <<
			                   " million words (optimized), >" <<
				((MAX_GPTYPE)/(2ULL*1024ULL*1024ULL)) << " million (non-optimized)" << endl <<
...
//cerr << "DEBUG Gla" << endl;
  ...
  logf (LOG_NOTICE, "BSn %s (Iindex) %s (%s)", prognam.c_str(), __IB_Version,  __DATE__ );
...
#ifndef _WIN32
  cout << "Note: If the IB software has NOT been installed in /opt/BSN please confirm that" << endl <<
