#pragma ident  "@(#)idb.cxx  1.235 06/07/01 02:41:09 BSN"
/************************************************************************
************************************************************************/
...
EMAIL_ADDRESS:    edz@bsn.com
ORGANIZATION: Basis Systeme netzwerk
*/
...
void IDB::SetIndexingMemory (const size_t MemorySize, GDT_BOOLEAN Force)
{
  ...
  if (MemorySize == 0)
    {
      const char msg[] = "Using (%s Memorysize) %lu";
      if (freeMemory <= 0)
	{
	  IndexingMemory = DefaultMemSize;
	  logf (LOG_DEBUG, msg, "Default", IndexingMemory);
	}
      else if ((IndexingMemory = physical/2) < MinimumMemSize)
	{
	  IndexingMemory = MinimumMemSize;
	  logf (LOG_DEBUG, msg, "Minimum", IndexingMemory);
	}
      else if ((UINT8)IndexingMemory > ((UINT8)DefaultMemSize)*4)
	{
	  if ((IndexingMemory = DefaultMemSize + physical/8) > ((UINT8)DefaultMemSize)*4)
	    {
	      IndexingMemory = DefaultMemSize*2;
	      logf (LOG_DEBUG, msg, "Double the Default", IndexingMemory);
	    }
	}
    }
...
Server=<Server Name, e.g. www.bsn.com>\n";
