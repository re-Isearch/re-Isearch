...
<< endl <<
#if 0
        "                   //   FILE:pattern      := Records whose path match pattern" << endl <<
        "                   //   EXTENSION:pattern := Records whose file extension match pattern" << endl <<
        "                   //   KEY:pattern       := Records whose key match pattern" << endl << 
        "                   //   DOCTYPE:pattern   := Records whose doctype match pattern" << endl <<
	"                   //     (where pattern is a glob/wildcard expression)" << endl << 
#endif
	"                   //   WITHIN[:daterange] := Record dates within the range" << endl <<
	"                   //   INSIDE[:field] := Hits are elements are in the specified field" << endl <<
...
int _Isearch_main (int argc, char **argv)
{
  ...
  if (argc < 2)
	...
	<< "Copyright \251 1995-2008 Basis Systeme netzwerk/Munich. All Rights Reserved."
	<< endl << endl;
...
#define TEST 0 
#if TEST
//  sQuery = "text/\"to be or not to\"";
//  sQuery = "money and fffooobar";
//  sQuery = "love and money";
//  sQuery = "xxxxxooxoxoxoxo";
 if (squery.SetInfixTerm(sQuery) == GDT_FALSE) cerr << "NOOOOOOOOOOOOOOOOOOOOOOOOOOOOPE" << endl;
#endif
