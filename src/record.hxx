...
public:
  ...
  bool Exists() const { return Pathname.Exists(); }
  ...
  void           SetBadRecord(bool bad = true) {
    if (bad) Segment = (Segment | 0x8000);
    else     Segment = (Segment & 0x7FFF);
  }
  bool    IsBadRecord() const  { return (Segment & 0x8000) != 0; }
  bool    SetSegment(int nSegment) {
...
inline bool Read (PRECORD RecordPtr, PFILE Fp)
{
