...
static const struct { const char *mongo; const char *reisearch; } MongoAliases[] = {
  /* SORTED */
  { "bool",          "boolean"   },
  ...
  { "oid",           "dotnumber" },  // 24-hex ObjectId string
...
static int CmpMongoAlias(const void *a, const void *b)
{
  return strcmp((const char *)a,
                ((const struct { const char *m; const char *r; } *)b)->mongo);
}
...
Examples:\n\
  { \"loc\":  { \"$box\":       \"51.5,-0.1,52,0.1\" } } -> box\n\
  { \"name\": { \"$metaphone\": \"Smith\"            } } -> metaphone\n\
  { \"id\":   { \"$oid\":       \"507f1f77...\"       } } -> dotnumber\n\
Options: see JSONDOC.";
}
                               ...
                               FIELDTYPE& ft, bool& skip) const
{
  ...
  // 1. Direct FIELDTYPE lookup — handles all re-Isearch types and aliases.
  //    This means $date, $numerical, $box, $iban, $metaphone, $phonetic,
  //    $boolean, $xs:dateTime, $dotnumber etc. all resolve automatically
  //    without any explicit entry in this file.
  ft = FIELDTYPE(nameBuf);
  if (ft.Defined())
    return true;

  // 2. MongoDB-specific alias table
  const auto *alias =
    (const struct { const char *m; const char *r; } *)bsearch(
      nameBuf, MongoAliases, MongoAliasCount,
      sizeof(MongoAliases[0]), CmpMongoAlias);
  if (alias)
    {
      ft = FIELDTYPE(alias->reisearch);
      return ft.Defined();  // should always be true given our table
    }

  // 3. Skip types — known MongoDB types with no indexable content
  ...
  // Unknown '$' key — not an EJSON wrapper we recognise; treat as
  // a normal nested object so we don't lose data.
  return false;
}
