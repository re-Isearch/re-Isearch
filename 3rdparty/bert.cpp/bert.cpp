...
//        int n_tensors = 0;
    ...
    // Calculate space requirements for setting up context buffers later
    {
        ...
        new_bert->mem_per_input = 1.2 * (new_bert->mem_per_token * N); // add 10% to account for ggml object overhead
...
static inline void bert_reset_compute_buffer(bert_ctx * ctx) {
    if (ctx->buf_compute.data) {
        free(ctx->buf_compute.data);
    }

    ctx->buf_compute.data = (uint8_t *) malloc(ctx->buf_compute.size);
    if (!ctx->buf_compute.data) {
        fprintf(stderr, "bert_reset_compute_buffer: malloc failed\n");
        abort();
    }
}
...
{
      struct ggml_init_params params = {
            .mem_size = ctx->buf_compute.size,
            .mem_buffer =ctx-> buf_compute.data,
            .no_alloc = false,
        };
    ...
    for (int ba = 0; ba < n_batch_size; ba++)
    {
        ...
        struct ggml_init_params params = {
         ...
         // CRITICAL FIX
        bert_reset_compute_buffer(ctx);
        params.mem_buffer = ctx->buf_compute.data;
        // END CRITICAL FIX 

       // fprintf(stderr, "bert_eval: compute buf size = %zu bytes\n", ctx->buf_compute.size);
	...
	if (!ctx0) {
        ...
        if (ggml_graph_compute_with_ctx(ctx0, &gf, n_threads)) {
