...
%newobject Fill;
  ...
  // Join sets
  IRSET *Join (const IRSET& OtherIrset);
  ...
  IRSET *Inclusive(const char *FieldName);
  ...
  IRSET *WithinFile(const char * FileSpec);
  IRSET *WithinDoctype(const char * DoctypeNameSpec);
  IRSET *WithKey(const char * KeySpec);
  ...
  IRSET *Trim (int Value=0);
  IRSET *HitCount (int Value=0);

  IRSET *BoostScore (float Weight = 1.0);
  ...
  // Special insertion
  void  setPrivateSortUserData(void *ptr);
  void *getPrivateSortUserData();
...
//  const char *Description();
  ...
  bool        UsePersistantCache() const;

  STRING      PersistantCacheName() const;
