...
%typemap(in) STRING {
  if (PyString_Check($input))
    $1 = STRING(PyBytes_AsString($input), PyBytes_Size($input));
  else
...
%typemap(in) const STRING& (STRING temp) {
  if (PyString_Check($input)) {
    temp = STRING(PyBytes_AsString($input), PyBytes_Size($input));
    $1 = &temp;
  } else {
    SWIG_exception(SWIG_TypeError,  StringErrorMsg);
  }
...
%typemap(directorout) STRING {
  if (PyString_Check($input))
    $result = STRING(PyBytes_AsString($input), PyBytes_Size($input));
  else
...
%typemap(in) STRING *{
   STRING tmp_string(PyBytes_AsString($source), PyBytes_Size($source));
   $target=&tmp_string;
}
...
static const char err_not_a_tuple[] = "Expected a tuple object.";
...
STRING* STRING_LIST_helper(PyObject* source) {
    ...
    if ((ptr = PyBytes_AsString(o)) != NULL && *ptr)
      temp[j++] = ptr;
    }
    temp[j] = NulString;
    return temp;
    ...
    for (int x=0; x<count; x++) {
      ...
      if (! PyString_Check(o)) {
        ...
        delete temp;
          ...
          temp->AddEntry(  PyBytes_AsString(o) );
        }
    return temp;
    }
    return NULL;
...
// Tuple to FC
// Added 2023
FC *FC_FromPyTuple(PyObject* source) {
    if (!PyTuple_Check(source))
      {
        PyErr_SetString(PyExc_TypeError, err_not_a_tuple);
      }
    else
      {
        PyObject* gp1 = PyTuple_GetItem(source, 0);
        PyObject* gp2 = PyTuple_GetItem(source, 1); 
        if (! PyInt_Check(gp1) || ! PyInt_Check(gp2)) {
            PyErr_SetString(PyExc_TypeError, "Expected a tuple of ints.");
            return NULL;
        }
        return new FC( PyInt_AsLong(gp1),  PyInt_AsLong(gp2) );
      }
    return NULL;
}
