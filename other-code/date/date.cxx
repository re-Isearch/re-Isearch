...
const int32_t INFINITE_DATE = 31129999;

const uint32_t YEAR_LOWER   =         1;
const uint32_t YEAR_UPPER   =      9999;
const uint32_t YEAR_FACTOR  =         1;

const uint32_t MONTH_LOWER  =     10000;
const uint32_t MONTH_UPPER  =    129999;
const uint32_t MONTH_FACTOR =     10000;

const uint32_t DAY_LOWER    =   1010000;
const uint32_t DAY_UPPER    =  31129999;
const uint32_t DAY_FACTOR   =   1000000;

const uint32_t DATE_ERROR   = 100000002;
const uint32_t DATE_UNKNOWN = 100000001;

const uint32_t DATE_PRESENT = 100000000;
...
SRCH_DATE::SRCH_DATE(const char* CStringVal)
{
  Set(CStringVal);
}
...
uint32_t SRCH_DATE::GetTime() const
{
  return (d_rest & 0x000FFFFF);
}

void SRCH_DATE::SetTime(uint32_t Time)
{
...
{
  d_rest = (d_rest & 0x000FFFFF) | ((uint32_t)(prec & 0xFF) << 20);
}
...
bool SRCH_DATE::Set(const char *CStringVal)
{
  bool res;
...
bool SRCH_DATE::SetTime(int hour, int min, int sec)
{
  if (hour >= 0 && hour < 24 && min >=0 && min < 60 && sec >=0 && sec < 60)
    {
      SetTime( (uint32_t)(hour*HOUR_FACTOR + min*MIN_FACTOR + sec*SEC_FACTOR));
      return true;
    }
  return false;
}
...
bool SRCH_DATE::SetTime(double Fraction)
{
  ...
  SetTime( (uint32_t)( ((t / 3600 ) % 24 )*HOUR_FACTOR + ((t / 60) % 60 )*MIN_FACTOR +
	( t % 60 )*SEC_FACTOR) );
  return true;
}
...
bool SRCH_DATE::IsDuring(const SRCH_DATE& OtherDate) const
{
  ...
  date1.SetTime((uint32_t)0);
  date2.SetTime((uint32_t)0);
...
DATERANGE::DATERANGE(const STRING& DateString)
{
  ...
  if (isdigit(DateString.at(0)) && (
	// Century or cent. or ...
	(seperator = DateString.SearchAny("cent")) > 2 ||
	(seperator = DateString.SearchAny("jahrhundred")) > 2 ||
	(seperator = DateString.SearchAny("jh.")) > 2 ||
	// 19�me Si�cle 
	((seperator = DateString.SearchAny("Si")) > 2 && DateString.at(seperator+2) == 'c') ) )
    {
      // TODO: Finish support of B.C. dates
      const int bc = ((DateString.Len() > (seperator+8) && DateString.at(seperator+7) == 'b') ? -100 : 100 );
      // 19th century
      // 12th century b.c.
  ...
  switch (end_range.GetLength())
	...
	// DD
	d_end = d_start;
	d_end.SetDay( std::stoi(end_range) );
	break;
      case 3:
	// MDD
	if (end_range.at(1) == '-')
	  {
	    //M-D
	    d_end = d_start;
	    d_end.SetDay  ( end_range.at(2) - '0'); 
	    d_end.SetMonth( end_range.at(0) - '0');
	    break;
	  }
	value = std::stoi(end_range);
	d_end = d_start;
	...
	if (end_range.at(1) == '-')
	  {
	    // M-DD
	    int day = (end_range.at(2) - '0')*10 +
		end_range.at(3) - '0';
	    int month = end_range.at(0) - '0';
	    d_end = d_start;
	    d_end.SetDay  ( day );
        ...
        if (end_range.at(2) == '-')
          {
	    // MM-D
	    int month = (end_range.at(0) - '0')*10 +
                end_range.at(1) - '0';
	    int day   = end_range.at(3) - '0';
	    d_end = d_start;
	    d_end.SetDay  ( day );
	    d_end.SetMonth( month );
	    break;
...
DATERANGE::DATERANGE(const char* Cstring)
{
